...
**图是用来解释文字的，所以它必须挨着它解释的那句话。** 把所有图集中在一节，
每张图确实都有正文讨论它，但没有一张图挨着自己的讨论——读者得记着图往回翻。
这不是配图，是画廊。

四条规则：

1. **就近**：图紧跟它支撑的那句话之后，不放章节末尾，不集中成组。
2. **一图一用**：每张图只出现一次，出现在它最承重的那个位置。
3. **图注说明它在这里干什么**，而不是复述 caption——「这张图支撑的是上面哪句话」。
4. **连续两张图之间必须有解释文字。** 这是硬规则，`readmap gate` 会阻断违反。
